...
usage() {
  cat <<'EOF'
Usage: scripts/verify-backup.sh [--snapshot ID|latest] [--keep] [--no-build]
...
restore_args=(--snapshot "$snapshot" --target-project "$target")
[[ "$skip_build" != true ]] || restore_args+=(--no-build)
"$ROOT/scripts/restore.sh" "${restore_args[@]}"
